...
async function verifyLicenseKey(licenseKey) {
  ...
  try {
    ...
    if (response.data && (response.data.valid === true || response.data.status === 'success')) {
      const clientName = response.data.client_name || (response.data.data ? response.data.data.client_name : null);
      return { valid: true, clientName: clientName };
    }
